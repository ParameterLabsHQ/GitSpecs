  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      ...
      run(async (item?: HubPrTreeItem) => {
        ...
        if (!branch) {
          void vscode.window.showInformationMessage("Select a PR with a head branch");
          return;
        }
        ...
        log.info(`Hub worktree ${branch} → ${path.trim()}`);
      }),
    ),
  );
}
